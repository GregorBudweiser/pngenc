...
int test_huffman_build_histogram() {
    ...
    huffman_codec_add(encoder.histogram, src, N);
...
int test_huffman_build_tree() {
    ...
    // Check minimal example
    {
        ...
        ASSERT_TRUE(encoder.codes[0] == 0)
        ASSERT_TRUE(encoder.codes[256] == 1)
    }
...
int unit_huffman(int argc, char* argv[])
{
    ...
    RETURN_ON_ERROR(test_huffman_decode())
