...
typedef struct _huffman_encoder {
    uint32_t histogram[HUFF_MAX_SYMBOLS];
    uint16_t codes[HUFF_MAX_SYMBOLS];       // max. 15 bit to represent value
    uint8_t code_lengths[HUFF_MAX_SYMBOLS]; // max.  4 bit to represent length
} huffman_codec;
...
int huffman_encoder_build_tree_limited(huffman_codec * encoder, uint8_t limit,
...
int huffman_encoder_encode64_3(const huffman_codec * encoder, const uint8_t * src,
                            uint32_t length, uint8_t * dst, uint64_t *offset);
...
int huffman_encoder_encode_full_simple(const huffman_codec * encoder_hist,
                                       const huffman_codec * encoder_dist,
